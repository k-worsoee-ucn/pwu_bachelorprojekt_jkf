...
1. **Backend Setup**
   ...
   # This command uses hardcoded credentials. If your local PostgreSQL has different credentials, update the DATABASE_URL in .env and run npm run prisma:push instead.
...
# Push schema to database (direct sync - dev only, accepts data loss)
npm run prisma:push
